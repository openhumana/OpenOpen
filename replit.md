...
- **Pricing Page**: Standalone Stripe-style pricing page at `/pricing` with three plans: Hire One Employee ($99/mo), Hire a Team ($399/mo), and Hire an Agency (custom/contact us). Uses white-labeled PayPal buttons for $99 and $399 plans; Agency card opens a lead-gen contact modal. CSS in `static/pricing.css`, JS in `static/pricing.js`. Nav dropdown links directly to `/pricing`.
