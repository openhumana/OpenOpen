...
- **Alex AI Chatbot**: An interactive chat widget on the landing page powered by Google Gemini (gemini-2.0-flash). Alex has a persistent personality as an ambitious job candidate, session-based conversation memory, and streaming responses. Backend in `alex_chat.py`, API at `/api/chat`, widget CSS in `static/chat-widget.css`.
