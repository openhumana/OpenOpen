...
## Overview
Open Humana is a production-ready, multi-user outbound voicemail drop web application. It enables users to upload voicemail audio and phone number lists through a modern dashboard. The system then automates the dialing process, detects answering machines, transfers human-answered calls, and drops pre-recorded voicemail messages. Built with Python, Flask, PostgreSQL, and the Telnyx Call Control API, it ensures data isolation per user and supports various authentication methods including email/password and Google OAuth. The project aims to provide intelligent communication at scale, offering advanced features like personalized voicemails, real-time call transcription, and comprehensive phone number management.
...
- **Branding**: "Open Humana" with a consistent dark color scheme (dark black, #1a1a1a) and Google-inspired success/error colors. The UI uses the Inter font and incorporates various visual effects on the landing page for an engaging user experience.
